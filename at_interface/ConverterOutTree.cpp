...
void ConverterOutTree::CopyParticle(const OutputContainer& kf_particle) {
  ...
  x_ = kf_particle.GetXDecay();
  y_ = kf_particle.GetYDecay();
  z_ = kf_particle.GetZDecay();
  x_err_ = kf_particle.GetXDecayError();
  y_err_ = kf_particle.GetYDecayError();
  z_err_ = kf_particle.GetZDecayError();
